...
class AppController(
    ...
    @PostMapping("{appId}/image", consumes = [MediaType.MULTIPART_FORM_DATA_VALUE])
    fun uploadImage(
        ...
        try {
            uploadAppImageUseCase.uploadImage(appId, requestMember.getId(), image)
    ...
    @DeleteMapping("{appId}/image")
    fun deleteImage(
        @PathVariable appId: Long,
        @AuthenticationPrincipal requestMember: AuthenticatedMember,
    ): ResponseEntity<Void> {
        try {
            deleteAppImageUseCase.deleteImage(appId, requestMember.getId())
        } catch (e: EntityNotFoundException) {
            throw HttpNotFoundException("App not found.")
        } catch (e: PermissionDeniedException) {
            throw HttpForbiddenException()
